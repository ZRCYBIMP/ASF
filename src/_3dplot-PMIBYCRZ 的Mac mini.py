...
class Plot3D(StandardFigure):
    ...
    def add_colorbar_3d(self, shrink=0.6, pad=0.15, ticks=None):
        """
        为3D图添加颜色条。
        ...
        cbar = self.fig.colorbar(mappable, ax=self.ax, shrink=shrink, pad=pad, ticks=ticks)
...
if __name__ == "__main__":
    ...
    elevation = 20
    azimuth = 150
    bounds = [sf.norm.vmin, sf.norm.vcenter, sf.norm.vmax]
    mappable = mpl.cm.ScalarMappable(norm=sf.norm, cmap=sf.cmap)
    # cbar = sf.create_colorbar(sf.ax)
    # cbar = sf.auto_adjust_colorbar(mappable=mappable)

    cbar = sf.add_colorbar_3d()
    sf.ax.zaxis._axinfo['juggled'] = (1,2,0)
    # ax.view_init(elev=elevation, azim=azimuth)
    # sf.fig.tight_layout() 
    ax_pos = sf.ax.get_position()

    sf.ax.set_position([ax_pos.x0+0.2*ax_pos.width, ax_pos.y0, ax_pos.width*0.9, ax_pos.height])  # 这些值也是以图形窗口的比例为单位
